...
@click.option("--end", type=str, required=True, help="End date in YYYY-MM-DD format")
@click.option(
    "--log-level",
...
@click.option("--save-data", is_flag=True, help="Save signal data to CSV")
@click.option(
    "--report-format",
...
def run_backtest_cli(
    ...
    try:
        ...
        signals_df = generate_signals_with_regime_filter(
            fx_series=data["fx"], comd_series=data["comd"], config=pair_config
        )

        trading_logger.logger.info(
            f"Generated {signals_df['signal'].abs().sum()} total signals"
        )
        ...
        print(
            "Good-regime %:",
            float(
                ((sigs["enter_long"]) | (sigs["enter_short"]) | (sigs["exit"])).mean()
                * 100
            ),
        )
        print(
            "Mean |z| @ entries:",
            float(
                sigs.loc[sigs["enter_long"] | sigs["enter_short"], "spread_z"]
                .abs()
                .mean()
            ),
        )
        print(
            "ADF p-value snapshot:",
            sigs["adf_p"] if isinstance(sigs["adf_p"], float) else sigs["adf_p"],
        )
        print(
            "Commodity big-gap days (|Δ|>5%):",
...
@cli.command()
@click.option("--pair", type=str, required=True, help="Pair name to show config for")
