...
def create_features(
    ...
    if "spread" in signals_df.columns:
        ...
        for period in lookback_periods:
            features[f"spread_mean_{period}"] = signals_df["spread"].rolling(period).mean()
            features[f"spread_std_{period}"] = signals_df["spread"].rolling(period).std()
...
def apply_ml_filter(
    ...
    if ml_filter.is_trained:
        ...
        logger.info(f"ML filter: {ml_filter_series.sum()}/{len(ml_filter_series)} signals accepted")
    else:
...
def create_training_labels(
    ...
    if "spread" in signals_df.columns:
        forward_returns = signals_df["spread"].pct_change(forward_return_window).shift(-forward_return_window)
    else:
        # Fallback to simple random labels
        forward_returns = pd.Series(np.random.randn(len(signals_df)), index=signals_df.index)
