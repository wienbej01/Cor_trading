...
def hurst_exponent(series: pd.Series, max_lags: int = 20) -> float:
    ...
    if len(clean_series) < 100:
        raise ValueError(
            "Series must have at least 100 non-NA data points for Hurst exponent"
        )
    ...
    try:
        ...
        tau = [
            np.var(np.subtract(clean_series[lag:], clean_series[:-lag])) for lag in lags
        ]
...
def is_cointegrated(
    ...
    try:
        ...
        logger.debug(
            f"Cointegration test: p-value={p_value:.6f}, is_cointegrated={is_coint}"
        )
