...
def rolling_beta(
    series_y: pd.Series, series_x: pd.Series, window: int = 20
) -> pd.Series:
    """
...
def zscore_robust(s: pd.Series, window: int) -> pd.Series:
    ...
    logger.info(
        f"zscore_robust called: series length={len(s)}, window={window}, NaN count={s.isna().sum()}"
    )

    roll = s.rolling(window)
    med = roll.median()
    logger.info(
        f"Rolling median computed: length={len(med)}, NaN count={med.isna().sum()}"
    )

    mad = roll.apply(
        lambda v: np.median(np.abs(v - np.median(v))) if len(v.dropna()) else np.nan
    )
    logger.info(f"MAD computed: length={len(mad)}, NaN count={mad.isna().sum()}")

    denominator = 1.4826 * (mad.replace(0, np.nan)) + 1e-12
    logger.info(
        f"Denominator computed: min={denominator.min()}, max={denominator.max()}"
    )

    z_scores = (s - med) / denominator
    logger.info(
        f"Z-scores computed: length={len(z_scores)}, NaN count={z_scores.isna().sum()}"
    )
