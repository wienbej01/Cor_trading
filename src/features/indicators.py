...
def atr(series: pd.Series, window: int) -> pd.Series:
    """
    Calculate Average True Range (ATR) proxy using percentage changes.
    ...
    return r.rolling(window).mean().bfill()


def rolling_corr(a: pd.Series, b: pd.Series, window: int) -> pd.Series:
    """
    Calculate rolling correlation between two series based on percentage changes.
    ...
    return a.pct_change().rolling(window).corr(b.pct_change())