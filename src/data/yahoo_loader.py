...
def download_daily(symbol: str, start: str, end: str, max_retries: int = 3, retry_delay: float = 1.0) -> pd.Series:
    """
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                logger.warning(f"Download failed for {symbol} (attempt {attempt + 1}): {e}. Retrying in {retry_delay}s...")
...
def align_series(series_a: pd.Series, series_b: pd.Series, method: str = "inner") -> pd.DataFrame:
    ...
    """
    logger.info(f"Aligning series: {series_a.name} ({len(series_a)} points) and {series_b.name} ({len(series_b)} points)")
    ...
    try:
        ...
        logger.debug(f"Original date ranges: {name_a}: {series_a.index.min()} to {series_a.index.max()}")
        logger.debug(f"Original date ranges: {name_b}: {series_b.index.min()} to {series_b.index.max()}")
        ...
        logger.info(f"Successfully aligned series with {len(aligned_df)} common data points")
        logger.debug(f"Aligned date range: {aligned_df.index.min()} to {aligned_df.index.max()}")
...
def download_and_align_pair(
    ...
    try:
        ...
        logger.info(f"FX data: {len(fx_series)} points from {fx_series.index.min()} to {fx_series.index.max()}")
        logger.info(f"Commodity data: {len(comd_series)} points from {comd_series.index.min()} to {comd_series.index.max()}")
        ...
        # Log final results
        logger.success(f"Successfully downloaded and aligned pair")
