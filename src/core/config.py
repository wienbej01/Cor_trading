...
class ConfigManager:
    ...
    def get_risk_config(self, pair_name: str) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dictionary containing risk configuration for the specified pair.
        """
        pair_config = self.get_pair_config(pair_name)
        return pair_config.get("risk", {})

    def get_execution_config(self, pair_name: str) -> Dict[str, Any]:
        """
        Get execution configuration for a specific pair.

        Args:
            pair_name: Name of the pair (e.g., "usdcad_wti").

        Returns:
            Dictionary containing execution configuration for the specified pair.
        """
        pair_config = self.get_pair_config(pair_name)
        # Execution config would typically be global, but we can have
        # pair-specific overrides
        return pair_config.get("execution", {})
